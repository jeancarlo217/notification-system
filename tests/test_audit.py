...
submitter's IP, the country Cloudflare reports, the timestamp and the identifier of the record
touched, with the address taken from Cloudflare's forwarding headers only) and the observability
rule of section 8 (logs are structured and carry their correlation keys, bound once per request
or run).
"""
...
def test_a_registration_is_audited_with_address_country_timestamp_and_record(
    client: Client,
    captured: _Captured,
) -> None:
    """I6: with no login, the audit entry is the only answer to who entered this."""
    client.post(reverse("service-create"), REGISTRATION, headers=CLOUDFLARE_HEADERS)
...
def test_the_address_is_never_invented_when_cloudflare_did_not_report_one(
    ...
    client.post(reverse("service-create"), REGISTRATION, REMOTE_ADDR="10.0.0.9")
