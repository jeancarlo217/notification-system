...
B1 (scaffold), B2 (configuration surface), B3 (data model), B4 (registration and lifecycle), B7
(alert engine) and B9 (scheduler) are done as of 2026-08-28; the B8 spike has Evolution API running
in Compose and waits on the owner pairing the phone (OQ-1 open). Application code so far: the
configuration boundary (`specs/adr/0001-configuration-boundary.md`), the two models with their
generated migrations, the registration screens (forms, views, templates) served at the site root
until B5, and the alert engine behind the provider interface with its `send_alerts` command and
scheduler service (`specs/adr/0002-scheduler-container.md`).
...
- Code: `deadliner/` is the Django project (settings, config, urls, wsgi, asgi), `core/` is the app
  (`models.py` with `Service` and `Alert`, `migrations/` generated by the CLI, `forms.py`,
  `views.py`, `urls.py`, `templates/core/`, `engine.py` and `provider.py` for the daily engine,
  `management/commands/send_alerts.py`), `manage.py` at the root. `tests/` holds every test; each
  names the identifier it implements.
...
- `just setup`: create `venv/` with Python 3.13 (`py -3.13` on Windows, `python3.13` elsewhere),
  install `requirements-dev.txt`, copy `.env.example` to `.env` if absent.
