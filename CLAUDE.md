...
catalogue), B13 (submitter identity), B14 (interface refactor, doing) and B15 (alert state in the
interface). Their shapes are decided ahead of the code in `specs/adr/0005-service-catalogue.md` and
`specs/adr/0006-submitter-identity.md`; the delivery order and the reason for it are in the
backlog.
...
- Code: `deadliner/` is the Django project (settings, config, urls, log_redaction, log_context,
  ...
  (`models.py` with `Service` and `Alert`, and with `ServiceCategory`, `CatalogService` and
  `Submitter` once B12 and B13 land, `migrations/` generated by the CLI, `forms.py`, `views.py`
  including the health endpoint, `urls.py`, `templates/core/`, `admin.py`, `widgets.py` for the
  combobox widgets, `identity.py` for name normalization, `audit.py`, `engine.py` and `provider.py`
  for the daily engine, `management/commands/send_alerts.py`), `manage.py` at the root. `tests/`
  holds every test; each names the identifier it implements.
...
- What a service is comes from the catalogue, never from free text. A record references the
  ...
  `choices=` on the field is the literal I4 forbids and the business renames services often.
