...
- Code: `deadliner/` is the Django project (settings, config, urls, log_redaction, log_context,
  ...
  `views.py` including the health endpoint, `urls.py`, `templates/core/`, `audit.py`, `engine.py`
  and `provider.py` for the daily engine, `management/commands/send_alerts.py`), `manage.py` at
  the root. `tests/` holds every test; each
