...
On the same day the owner also split delivery in two phases: phase one is registration and the
export with no WhatsApp at all, because nothing is worth notifying about until the registry has
something in it, so the `scheduler` service does not deploy and OQ-3's wording stops gating go live.
That made B10 (the CSV export, done) the only way data leaves the system and opened B22, the
database backup with a restore that was actually executed, since the registry phase one accumulates
...
- Code: `deadliner/` is the Django project (settings, config, urls, log_redaction, log_context,
  ...
  `export.py` for the CSV row, `backups.py` for the copy and its retention, `audit.py`,
  `engine.py` and `provider.py` for the daily engine, `management/commands/send_alerts.py` and
  `backup_database.py`, `static/` with the brand assets), `manage.py` at the root.
  `backups/` holds the copies and is ignored except for its `.gitkeep`.
...
- The database is copied daily and the restore has been executed, not merely written (B22). A
  backup nobody has restored from is a claim, and copies on the same disk do not survive losing it.
...
- `just gate`: lint, typecheck, test, secret-scan, in CI order. Green here is the precondition for
  a commit.
- `just up` / `just down`: the Compose stack (web on port 8000, the daily scheduler, the daily
  backup, SQLite on the `data` volume, Evolution API on 127.0.0.1:8080 with its own Postgres and
  Redis). Production runs `web backup` and deliberately not `scheduler`, which would fail hourly
  until the B8 adapter exists; the README says so where a deployer meets it.
- `just backup`: copy the database now into `./backups`. `just restore <file>`: put one back, which
