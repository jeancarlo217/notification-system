...
(secret path and health endpoint), B7 (alert engine) and B9 (scheduler) are done as of 2026-08-28;
...
(`specs/adr/0003-secret-path-and-log-redaction.md`), and the alert engine behind the provider
...
- Code: `deadliner/` is the Django project (settings, config, urls, log_redaction, wsgi, asgi),
  `core/` is the app
  (`models.py` with `Service` and `Alert`, `migrations/` generated by the CLI, `forms.py`,
  `views.py` including the health endpoint, `urls.py`, `templates/core/`, `engine.py` and
  `provider.py` for the daily engine, `management/commands/send_alerts.py`), `manage.py` at the
  root. `tests/` holds every test; each
