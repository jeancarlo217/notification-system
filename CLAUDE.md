...
(secret path and health endpoint), B6 (audit and structured logging), B7 (alert engine), B9
(scheduler) and B10 (CSV export) are done as of 2026-08-28;
...
- Code: `deadliner/` is the Django project (settings, config, urls, log_redaction, log_context,
  ...
  `views.py` including the health endpoint and the export, `urls.py`, `templates/core/`,
  `audit.py`, `export.py`, `engine.py` and `provider.py` for the daily engine,
