...
**This ecosystem's rule is stricter than the generic one and it lives in `CLAUDE.md`, not here.** A comment
is a **trap, not an explanation**: it earns its place only when the correct code looks wrong, or the wrong
code looks right, so that without the note somebody "fixes" the line and reintroduces the defect.
**"Explains why" is not the bar**, because every line has a why, and a rule that admits every line is a
rule that produces a comment on every line. The default is none.
...
// BAD: Explains why, and nobody was going to touch this line anyway
// Increment so the next iteration reads the following record
counter++;

// EARNS ITS PLACE: the correct code looks wrong, and deleting the note invites the "fix"
// The upstream API is 0-based and this counter is published 1-based
counter++;
```
